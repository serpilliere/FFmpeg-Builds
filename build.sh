...
cat <<EOF >"$BUILD_SCRIPT"
    ...
    patch -p0 < /patches/custom/patch_remove_deps
...
docker run --rm -i $TTY_ARG "${UIDARGS[@]}" -v "$PWD/ffbuild":/ffbuild -v "$PWD/patches":/patches -v "$BUILD_SCRIPT":/build.sh "$IMAGE" bash /build.sh
